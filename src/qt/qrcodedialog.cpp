...
{
    QString ret = QString("pingancoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non PAC unit input in lnReqAmount, we generate the URI with PAC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::PAC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
