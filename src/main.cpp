...
uint256 hashGenesisBlock("0x057dccf86ac25a04bde290bb5d240d689ff12f90d39bc1c9f2c58dd3955fb3d5");
...
const string strMessageMagic = "PinganCoin Signed Message:\n";
...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uPAC
...
static const int64 nInterval = nTargetTimespan / nTargetSpacing; // 30 minutes retarget
...
void ThreadScriptCheck() {
    RenameThread("pingancoin-scriptch");
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = " Beijing Times 13/Apr/2016 luoguojun,create pingancoin in PA XJK";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1460544000;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1180969;

		if (fTestNet)
        {
            block.nTime    = 1460543700;
            block.nNonce   = 0;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x15e12c4c3963e375d602e43ac3635b61a08036890cdb6e8c005cf8a181a98a29"));
        ...
        if (false && block.GetHash() != hashGenesisBlock)
        {
			...
			printf("*** GENESIS BLOCK CREATION COMPLETE ***");
...
unsigned char pchMessageStart[4] = { 0x32, 0xf1, 0x24, 0x23 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PingancoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PingancoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("PingancoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pingancoin-miner");
    ...
    try { while (true) {
        ...
        printf("Running PingancoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PingancoinMiner terminated\n");
        throw;
    }
}
